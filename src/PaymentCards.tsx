...
import { useGetPackageCardsQuery } from './store/services/paymentService';
import { useNavigate } from 'react-router-dom';

const PaymentCards = () => {
    const navigate = useNavigate();
    const { data, isLoading, error } = useGetPackageCardsQuery();
        ...
        <div className="md:flex justify-between space-y-4 md:space-y-0 md:space-x-4 rtl:space-x-reverse">
           {
            data?.data.map((card) => (
                ...
                <ul className="space-y-2.5 mb-5 font-semibold group-hover:text-primary">
                   {card.packageServices.map((service) => (
                     <li className="flex justify-center items-center" key={service.id}>
                     <FaArrowLeft/>
                     {service.packageService.name} - {service.packageService.price.toLocaleString()} Ksh for {service.packageService.durationDays} days
                 </li>
                   ))}
